...
Deno.serve(async (request) => {
  ...
  const { reportId, action, reason } = (await request
    .json()
    .catch(() => ({}))) as {
    reportId?: string
    action?: 'resolved' | 'expired' | 'reopen' | 'delete'
    reason?: string
  }
  if (
    !reportId ||
    !action ||
    !['resolved', 'expired', 'reopen', 'delete'].includes(action)
  )
    return response(request, 400, {
      error: 'A report and valid action are required.',
    })
  ...
  if (allowed !== true)
    return response(request, 403, {
      error: 'Only Pet Seen staff can manage reports.',
    })
  if (action !== 'delete') {
    const status = action === 'reopen' ? 'active' : action
    const { error } = await user.rpc('set_found_pet_report_lifecycle', {
      target_report_id: reportId,
      next_status: status,
      reason: action === 'reopen' ? null : reason?.trim() || null,
    })
  ...
  const paths = [photo?.source_object_path, photo?.display_object_path].filter(
  ...
  if (paths.length) {
    ...
    if (error)
      return response(request, 502, {
        error: 'Report files could not be deleted.',
      })
    await admin
      .from('found_pet_report_moderation_audit')
      .insert({ found_pet_report_id: reportId, event: 'deleted_files_deleted' })
  }
  await admin.from('found_pet_report_moderation_audit').insert({
    found_pet_report_id: reportId,
    event: 'deleted',
    metadata: { reason: reason?.trim() || 'staff_cleanup' },
  })
  ...
  if (error)
    return response(request, 502, {
      error: 'Report content could not be deleted.',
    })
