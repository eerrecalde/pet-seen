...
Deno.serve(async (request) => {
  ...
  if (!slug || !/^[a-z0-9]{10,32}$/.test(slug))
  ...
  const { data } = await client
    ...
    .select('pet_name,species,breed,colour,last_seen_description')
    .eq('public_slug', slug)
    .maybeSingle()
  if (!data) return new Response('Not found', { status: 404 })
  const petName = escape(data.pet_name)
  const details = escape(
    [data.colour, data.breed].filter(Boolean).join(' · ') || data.species,
  )
  const area = escape(data.last_seen_description || 'Approximate area')
