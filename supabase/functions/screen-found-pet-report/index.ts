...
Deno.serve(async (request) => {
  ...
  if (!url || !serviceKey)
    return response(request, 500, { error: 'Screening is unavailable.' })
  const { reportId, submissionToken } = (await request
    .json()
    .catch(() => ({}))) as { reportId?: string; submissionToken?: string }
  if (!reportId || !submissionToken)
    return response(request, 400, {
      error: 'A report and submission token are required.',
    })
  const admin = createClient(url, serviceKey)
  const { data: report } = await admin
    .from('found_pet_reports')
    .select(
      'id,breed,colour,details,location_description,custody_details,client_submission_id,moderation_status,photo:found_pet_photos(display_object_path)',
    )
    .eq('id', reportId)
    .maybeSingle()
  if (!report || report.client_submission_id !== submissionToken)
    return response(request, 404, { error: 'Found-pet report not found.' })
  if (report.moderation_status !== 'pending')
    return response(request, 200, { status: report.moderation_status })
  try {
    ...
    try {
      ...
      console.warn(
        'Automatic found-pet screening attempt failed; retrying once',
        {
          reportId,
    ...
    const note = result.categories.length
      ? `OpenAI moderation flagged: ${result.categories.join(', ')}`
      : 'OpenAI moderation passed'
    await admin
      .from('found_pet_reports')
      .update({
    ...
    if (result.flagged) {
      const { data: storedPhoto } = await admin
      ...
      if (storedPhoto)
        await admin.storage
          ...
          .remove(
            [
              storedPhoto.source_object_path,
              storedPhoto.display_object_path,
            ].filter(Boolean),
          )
      await admin.from('found_pet_report_moderation_audit').insert({
        found_pet_report_id: reportId,
    ...
    const note =
      'Automatic safety screening could not complete. This report needs staff review.'
    await admin
      .from('found_pet_reports')
      .update({ automated_screening_note: note })
      .eq('id', reportId)
      .eq('moderation_status', 'pending')
    await admin.from('found_pet_report_moderation_audit').insert({
      found_pet_report_id: reportId,
      event: 'automatic_screening_failed',
      automated_screening_note: note,
      metadata: { stage: 'automatic_screening' },
    })
