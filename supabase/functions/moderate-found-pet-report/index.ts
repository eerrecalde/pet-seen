...
Deno.serve(async (request) => {
  ...
  const { reportId, decision } = await request.json().catch(() => ({})) as { reportId?: string, decision?: 'approved' | 'rejected' }
  if (!reportId || !['approved', 'rejected'].includes(decision ?? '')) return response(request, 400, { error: 'A report and decision are required.' })
  ...
  if (allowed !== true) return response(request, 403, { error: 'Only Pet Seen staff can moderate reports.' })
  const admin = createClient(url, serviceKey)
  if (decision === 'rejected') {
    const { data: photo } = await admin.from('found_pet_photos').select('source_object_path,display_object_path').eq('found_pet_report_id', reportId).maybeSingle()
    if (photo) { const paths = [photo.source_object_path, photo.display_object_path].filter(Boolean); const { error } = await admin.storage.from('found-pet-photos').remove(paths); if (error) return response(request, 502, { error: 'Rejected photo files could not be deleted.' }) }
  }
  ...
  if (decision === 'rejected') {
    ...
    if (deleteError) return response(request, 502, { error: 'Rejected report content could not be deleted.' })
  }
