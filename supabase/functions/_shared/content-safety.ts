...
type OpenAiModerationResponse = { results?: Array<{ flagged?: boolean, categories?: Record<string, boolean> }> }
...
class OpenAiModerationProvider implements ContentSafetyProvider {
  ...
  async screen(input: ContentSafetyInput): Promise<ContentSafetyResult> {
    ...
    if (!result || typeof result.flagged !== 'boolean') throw new Error('OpenAI moderation response was incomplete')
    return { flagged: result.flagged, categories: Object.entries(result.categories ?? {}).filter(([, flagged]) => flagged).map(([category]) => category) }
  }
}
...
export function contentSafetyProvider(): ContentSafetyProvider {
  ...
  if (provider !== 'openai') throw new Error(`Unsupported content safety provider: ${provider}`)
