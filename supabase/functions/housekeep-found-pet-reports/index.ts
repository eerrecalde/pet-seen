...
Deno.serve(async (request) => {
  ...
  if (!url || !serviceKey || !token)
    return response(request, 401, { error: 'Authorisation is required.' })
    ...
    if (paths.length) {
      ...
      if (error)
        return response(request, 502, {
          error: `Retention cleanup stopped: ${error.message}`,
        })
      await admin.from('found_pet_report_moderation_audit').insert({
        found_pet_report_id: report.report_id,
        event: 'retention_files_deleted',
      })
    }
    await admin.from('found_pet_report_moderation_audit').insert({
      found_pet_report_id: report.report_id,
