...
Deno.serve(async (request) => {
  ...
  if (!url || !serviceKey || !token)
    return reply(request, 401, { error: 'Sign in is required.' })
  if (!apiKey)
  ...
  if (!sightingId)
    return reply(request, 400, { error: 'An unlinked sighting is required.' })
  ...
  const { data: candidateData, error: candidateError } = await user.rpc(
    'unlinked_sighting_case_candidates',
    { target_sighting_id: sightingId },
  )
  if (candidateError)
    return reply(request, 400, { error: candidateError.message })
  ...
  const { data: sighting } = await admin
    .from('sightings')
    .select('details,location_description')
    .eq('id', sightingId)
    .is('case_id', null)
    .maybeSingle()
  if (!actor.user || !sighting)
    return reply(request, 404, {
      error: 'This unlinked sighting is not available for scoring.',
    })
  const { data: run, error: runError } = await admin
    .from('ai_unlinked_sighting_match_runs')
    .insert({
  ...
  try {
    ...
    const openai = await fetch('https://api.openai.com/v1/responses', {
      ...
      body: JSON.stringify({
        ...
        text: {
          format: {
            type: 'json_schema',
            name: 'sighting_candidate_scores',
            strict: true,
            schema,
          },
        },
    ...
    const parsed = JSON.parse(
      outputText((await openai.json()) as ResponsesPayload) || '{}',
    ) as { scores?: Result[] }
    if (
      !parsed.scores ||
      parsed.scores.length !== candidates.length ||
      new Set(parsed.scores.map((score) => score.case_id)).size !==
        candidates.length ||
      parsed.scores.some(
        (score) =>
          !candidates.some((candidate) => candidate.case_id === score.case_id),
      )
    )
    ...
    const rows = parsed.scores.map((score) => {
      ...
      const ai = Math.round(Math.max(0, Math.min(100, score.similarity_score)))
      const combined = Math.round(candidate.match_score * 0.65 + ai * 0.35)
      return {
        run_id: run.id,
        sighting_id: sightingId,
        case_id: score.case_id,
        deterministic_score: candidate.match_score,
        ai_similarity_score: ai,
    ...
    const { error } = await admin
      .from('ai_unlinked_sighting_match_scores')
      .insert(rows)
