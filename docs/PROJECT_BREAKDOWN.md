...
| PS-426 | Add AI cost and abuse controls: move automatic found-pet candidate scoring to a durable, rate-limited queue; require a conservative deterministic shortlist/threshold before invoking the model; make scoring idempotent per report version; cap candidate images and input dimensions/bytes; define daily and monthly provider budget guardrails with a safe staff-review fallback; and record privacy-safe per-run token/latency/outcome metrics with tests for every skip, cap and retry path. | Done |
