...
| ID     | Task                                                                                                                                                                                                                                                   | Status |
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------ |
| PS-101 | Configure magic-link authentication and lightweight profiles, with a local-only development bypass and designated owner/staff test accounts for testing protected paths. Signed-in users land on their dashboard, with account controls in the header. | Done   |
...
| PS-105 | Build missing-case creation and location-picker flow with GPS, movable-pin confirmation and manual fallback; publish automatically after an owner confirms the required last-seen location                                                             | Done   |
...
| PS-422 | Add postcode/address search and explicit location confirmation to found-pet and anonymous-sighting reports: never submit the London fallback pin as a real location; support GPS, postcode/place selection and manual pin adjustment; and require a deliberate location selection while retaining a non-map fallback when location permission is unavailable.                                                                                                                                                                                 | Done        |
| PS-423 | Add an explicit, progressive “search a wider area” control to nearby discovery (for example 1, 3 and 5 miles), retaining bounded PostGIS queries and never loading additional cases merely because a visitor zooms the map.                                                                                                                                                                                                                                                                                                                   | Not started |
| PS-424 | Add an optional, accessible client-side photo-adjustment step to pet and found-pet uploads, using a mobile-capable cropper (for example `react-easy-crop`): after a user selects a JPEG or PNG, show a responsive adjustment view that lets them drag to position the animal and pinch or use a visible slider to zoom, with Reset, Skip, Replace and “Use this photo” controls. Keep this a crop-only interaction—no filters, retouching or AI removal—and do not force a crop when the original framing is already suitable. Render the confirmed crop as the JPEG that enters the existing validation, EXIF-removal, private/public display-derivative and moderation workflows; discard the browser-side original after replacement or confirmation. Preserve the existing direct/no-crop path, validation errors, 10 MB input limit and 2048 px maximum dimension. Ensure touch, mouse, keyboard focus, visible focus, screen-reader labels, reduced-motion behaviour and narrow mobile layouts work. Verify locally with the development Supabase/Edge Function runtime and the documented auth bypass: upload, crop, skip, replace and submit a missing-pet photo as `owner@petseen.org:owner`; then upload, crop, skip, replace and submit an optional found-pet photo and confirm it appears only in the authorised owner/moderator workflow. Inspect desktop and mobile rendered flows, including the final stored/display image dimensions and absence of original EXIF. Review the existing Playwright core-loop and found-pet coverage; update or add stable fixture-based E2E assertions for opening the adjustment step, submitting a crop, skipping it, replacing an image, and successfully completing each affected workflow without relying on fragile gesture coordinates. | In progress |
| PS-425 | Add notification fan-out controls for watch-area alerts: enforce a documented per-sighting recipient ceiling and per-owner watch-area limit; coalesce overlapping watch-area matches to one recipient; add a quiet period and/or digest path for repeated nearby sightings; prefer web push and rate-limit email fallback; retain durable retry semantics without allowing a single report to generate an unbounded number of provider sends; and add database/function tests for caps, deduplication and failure recovery. | Done        |
| PS-426 | Add AI cost and abuse controls: move automatic found-pet candidate scoring to a durable, rate-limited queue; require a conservative deterministic shortlist/threshold before invoking the model; make scoring idempotent per report version; cap candidate images and input dimensions/bytes; define daily and monthly provider budget guardrails with a safe staff-review fallback; and record privacy-safe per-run token/latency/outcome metrics with tests for every skip, cap and retry path. | In progress |
| PS-427 | Replace direct use of the public Nominatim endpoint with the selected MapTiler geocoding service behind a server-owned adapter or controlled client boundary: restrict production keys by origin, debounce and cancel searches, cache normalised query results, apply per-client and global request limits, preserve manual-pin fallback, and document provider quota, alert and failover configuration. | Not started |
| PS-428 | Investigate and optimise public pet-photo delivery for viral traffic: assess TanStack Query for caching public-case metadata and resolved image URLs within a browser session, including sensible stale times and invalidation; separately assess browser/CDN/Storage caching because TanStack Query does not cache image bytes between visitors or remove Edge Function/Storage delivery work. Implement the chosen layered approach: generate a small card-thumbnail derivative alongside the existing display image; serve public case and nearby-list images through cacheable, bounded public URLs without creating a fresh signed URL for every view; set safe cache headers and invalidation on case/photo changes; preserve the rule that source uploads and unpublished photos are never public; and verify browser, CDN and Storage egress behaviour with automated coverage. | Not started |
