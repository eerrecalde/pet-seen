...
export function NearbyDiscoveryMap({
  ...
  const pointsKey = points
    .map(
      ({ id, latitude, longitude, label, kind }) =>
        `${id}:${latitude}:${longitude}:${label}:${kind}`,
    )
    .join('|')
  ...
  useEffect(() => {
    ...
    const caseAreas = currentPoints
      .filter((point) => point.kind === 'case')
      .map((point) => ({
        ...approximateArea(point.latitude, point.longitude).data,
        properties: { label: point.label },
      }))
    instance.on('load', () => {
      instance.addSource('nearby-case-areas', {
        type: 'geojson',
        data: { type: 'FeatureCollection', features: caseAreas },
      })
      instance.addLayer({
        id: 'nearby-case-areas-fill',
        type: 'fill',
        source: 'nearby-case-areas',
        paint: { 'fill-color': '#d76f4e', 'fill-opacity': 0.3 },
      })
      instance.addLayer({
        id: 'nearby-case-areas-outline',
        type: 'line',
        source: 'nearby-case-areas',
        paint: { 'line-color': '#9b553a', 'line-width': 2 },
      })
      instance.on('mouseenter', 'nearby-case-areas-fill', () => {
        instance.getCanvas().style.cursor = 'pointer'
      })
      instance.on('mouseleave', 'nearby-case-areas-fill', () => {
    ...
    markers.current = currentPoints
      .filter((point) => point.kind === 'sighting')
      .map((point) => {
  ...
  return (
    <div
      className="nearby-discovery-map"
      ref={container}
      role="img"
      aria-label={label}
    />
