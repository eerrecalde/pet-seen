...
export function OwnerSightingMap({ points }: { points: SightingMapPoint[] }) {
  ...
  useEffect(() => {
    ...
    markers.current = currentPoints.map((point) =>
      new maplibregl.Marker({ color: '#47724a' })
  ...
  return (
    <div
      className="owner-sighting-map"
      ref={container}
      aria-label="Exact sighting locations for this case"
    />
  )
}
