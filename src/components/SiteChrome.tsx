...
export function Link({ to, ...props }: ComponentProps<typeof RouterLink>) {
  ...
  return (
    <RouterLink
      {...props}
      to={localisedPath(to.toString(), i18n.resolvedLanguage)}
    />
  )
}
...
export function PetSeenMark() {
  return (
    <svg className="wordmark-mark" viewBox="0 0 48 48" aria-hidden="true">
      <ellipse
        cx="7.5"
        cy="20.4"
        rx="5.2"
        ry="7"
        transform="rotate(-31 7.5 20.4)"
      />
      <ellipse
        cx="16.8"
        cy="9.7"
    ...
    <label className="language-picker">
      ...
      <select
        ...
        onChange={(event) => {
          ...
          navigate(
            `${localisedPath(path, locale)}${location.search}${location.hash}`,
          )
        }}
      >
        <option value="en-GB">{t('language.english')}</option>
        <option value="es-419">{t('language.spanish')}</option>
      </select>
    </label>
  )
}
...
export function SiteHeader() {
  ...
  return (
    <header className="site-header">
      <Link className="wordmark" to="/" aria-label={t('common.petSeenHome')}>
        <PetSeenMark />
        Pet Seen
      </Link>
      <nav aria-label={t('common.petSeenHome')}>
        <Link className="nearby-link" to="/#nearby">
          <Icon name="map-pin-2" />
          {t('common.nearbyPets')}
        </Link>
        <LanguagePicker />
          ...
          <details className="account-menu">
            <summary>
              ...
              <span className="account-email">
                {session.user.email ?? t('common.account')}
              </span>
...
export function Progress({
  ...
  return (
    <div className="progress" aria-label={step}>
  ...
  return (
    <header className="simple-header">
      <Link
        className="wordmark"
        to="/"
        aria-label={t('common.petSeenHome')}
        onClick={leaveFlow}
      >
        <PetSeenMark />
        Pet Seen
      ...
      <LanguagePicker />
      <Link className="back-link small-back" to="/" onClick={leaveFlow}>
