...
export function ModerationPage() {
  ...
  const loadReports = useCallback(async () => {
    setState('loading')
    try { const data = await moderationApi.load()
    setReports((data.content ?? []).map((report) => { const caseData = Array.isArray(report.case) ? report.case[0] ?? null : report.case; return { ...report, case: caseData ? { ...caseData, pet: Array.isArray(caseData.pet) ? caseData.pet[0] ?? null : caseData.pet } : null } }) as unknown as ModerationReport[])
    setFoundReports((data.found ?? []).map((report) => {
    ...
    setUnlinkedSightings((data.sightings ?? []).map((sighting) => ({ ...sighting, ai_scores: (sighting.ai_scores ?? []).sort((a, b) => b.created_at.localeCompare(a.created_at)) })) as UnlinkedSighting[])
    setState('ready') } catch { setState('error') }
  }, [])
  useEffect(() => { if (!session) { setAccess('denied'); return }; void moderationApi.isAuthorized().then((data) => { const allowed = data === true; setAccess(allowed ? 'allowed' : 'denied'); if (allowed) void loadReports() }).catch(() => setAccess('denied')) }, [loadReports, session])
  async function updateStatus(report: ModerationReport, status: ContentReportStatus) { setSavingId(report.id); try { await moderationApi.updateContentStatus(report.id, status); await loadReports() } finally { setSavingId(null) } }
  async function runHousekeeping() { setHousekeeping('running'); try { await moderationApi.housekeeping(); setHousekeeping('idle'); await loadReports() } catch { setHousekeeping('error') } }
...
function UnlinkedSightingMatches({ sightings, locale, onLinked }: { sightings: UnlinkedSighting[], locale: AppLocale, onLinked: () => Promise<void> }) {
  ...
  async function showCandidates(sightingId: string) { if (candidates[sightingId]) return; setLoadingId(sightingId); try { const result = await moderationApi.sightingCandidates(sightingId); setCandidates((current) => ({ ...current, [sightingId]: (result ?? []) as MatchCandidate[] })) } catch (error) { setError(error instanceof Error ? error.message : 'We could not find candidates.') } finally { setLoadingId(null) } }
  async function analyse(sightingId: string) { setAnalysingId(sightingId); setError(''); try { await moderationApi.scoreSighting(sightingId); await onLinked() } catch (error) { setError(error instanceof Error ? error.message : 'We could not analyse this sighting.') } finally { setAnalysingId(null) } }
  async function link(sightingId: string, caseId: string) { setLinkingId(`${sightingId}-${caseId}`); try { await moderationApi.linkSighting(sightingId, caseId); await onLinked() } catch (error) { setError(error instanceof Error ? error.message : 'We could not link this sighting.') } finally { setLinkingId(null) } }
  return <section className="found-match-section unlinked-sighting-section" aria-labelledby="unlinked-sightings-title"><div className="found-match-heading"><div><h2 id="unlinked-sightings-title">Unlinked sightings</h2><p>Review nearby cases before running the AI description analysis. Scores guide your review; staff can link any listed candidate and notify its owner.</p></div></div>{error && <p className="form-error">{error}</p>}{sightings.length === 0 ? <section className="dashboard-empty"><h3>No unlinked sightings</h3><p>New reports without a selected case will appear here for staff review.</p></section> : <div className="found-match-list">{sightings.map((sighting) => <article className="found-match-card" key={sighting.id}><div><p className="moderation-status">Reported · {formatDateTime(sighting.seen_at, locale)}</p><h3>{sighting.location_description || 'Location shared privately'}</h3><p>{sighting.details}</p></div><div className="found-match-actions"><button className="secondary-button" type="button" disabled={loadingId === sighting.id} onClick={() => void showCandidates(sighting.id)}>{loadingId === sighting.id ? 'Finding nearby cases…' : 'Find nearby cases'}</button>{candidates[sighting.id] && <><button className="secondary-button" type="button" disabled={analysingId === sighting.id} onClick={() => void analyse(sighting.id)}>{analysingId === sighting.id ? 'Analysing description…' : 'Run AI analysis'}</button><p className="ai-review-note">AI results are staff-only guidance. You decide whether to link a listed candidate.</p><SightingCandidateList candidates={candidates[sighting.id]} scores={sighting.ai_scores} sightingId={sighting.id} linkingId={linkingId} locale={locale} onLink={link} /></>}</div></article>)}</div>}</section>
}
...
function FoundPetMatches({ reports, locale, onLinked }: { reports: FoundPetReport[], locale: AppLocale, onLinked: () => Promise<void> }) {
  ...
  const photoUrls = useSignedPhotoUrls(reports.map((report) => ({ id: report.id, path: report.photo?.display_object_path ?? report.photo?.source_object_path ?? null })), moderationApi.signedPhoto)
  const [error, setError] = useState('')
  async function showCandidates(reportId: string) { if (candidates[reportId]) return; setLoadingId(reportId); try { const result = await moderationApi.foundCandidates(reportId); setCandidates((current) => ({ ...current, [reportId]: (result ?? []) as MatchCandidate[] })) } catch (error) { setError(error instanceof Error ? error.message : 'We could not find candidates.') } finally { setLoadingId(null) } }
  async function link(reportId: string, caseId: string) { setLinkingId(`${reportId}-${caseId}`); try { await moderationApi.linkFound(reportId, caseId); await onLinked() } catch (error) { setError(error instanceof Error ? error.message : 'We could not link this report.') } finally { setLinkingId(null) } }
  async function analyse(reportId: string) {
    setAnalysingId(reportId); setError('')
    try { await moderationApi.scoreFound(reportId); await onLinked() } catch (error) { setError(error instanceof Error ? error.message : 'We could not analyse this report.') } finally { setAnalysingId(null) }
  }
  async function review(reportId: string, decision: 'approved' | 'rejected') {
    setReviewingId(reportId); setError('')
    try { await moderationApi.reviewFound(reportId, decision); await onLinked() } catch (error) { setError(error instanceof Error ? error.message : 'We could not review this report.') } finally { setReviewingId(null) }
  }
...
function LifecycleActions({ report, onComplete }: { report: FoundPetReport, onComplete: () => Promise<void> }) {
  ...
  async function manage(action: 'resolved' | 'expired' | 'reopen' | 'delete') {
    ...
    try { await moderationApi.manageFound(report.id, action, reason); await onComplete() } catch (error) { setError(error instanceof Error ? error.message : 'We could not manage this report.') } finally { setSaving(false) }
  }
