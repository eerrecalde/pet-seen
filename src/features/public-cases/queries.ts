...
import { fetchNearbyDiscovery, fetchPublicCase, fetchPublicCaseOptions } from './api'

export const publicCaseQuery = (slug: string) => queryOptions({
  queryKey: queryKeys.publicCases.detail(slug),
  queryFn: () => fetchPublicCase(slug),
})

export const nearbyDiscoveryQuery = () => queryOptions({
  queryKey: queryKeys.publicCases.nearbyDiscovery(),
  queryFn: fetchNearbyDiscovery,
  staleTime: 60_000,
})

export const publicCaseOptionsQuery = () => queryOptions({
