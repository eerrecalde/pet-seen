...
function WatchAreas({ sessionUserId }: { sessionUserId: string }) {
  ...
  async function useLocation() { setStatus(''); const result = await getCurrentCoordinates({ maximumAge: 60_000, timeout: 15_000 }); if (!result.ok) { setStatus(result.reason === 'unavailable' ? 'Your browser cannot provide a location.' : 'We could not access your location. Check your browser permission and try again.'); return }; setPosition(result.coordinates); setStatus('Location selected. Give this area a name before saving.') }
