...
export async function preparePetPhoto(file: File): Promise<File> {
  ...
  return new File([blob], `${file.name.replace(/\.[^.]+$/, '') || 'pet-photo'}.jpg`, { type: 'image/jpeg', lastModified: Date.now() })
}
...
function loadImage(file: File) {
  return new Promise<HTMLImageElement>((resolve, reject) => {
    ...
    image.onerror = () => { URL.revokeObjectURL(url); reject(new PetPhotoError('decode')) }
    image.src = url
  })
}
