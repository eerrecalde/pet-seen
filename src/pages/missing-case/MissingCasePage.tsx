...
export function MissingCasePage() {
  ...
  async function savePet(event: FormEvent<HTMLFormElement>) {
    ...
    const { data, error: saveError } = await supabase.functions.invoke<{
      ...
      body: {
        kind: 'missing_case_draft',
        payload: {
          pet_name: petName,
          pet_species: species,
          pet_breed: String(fields.get('breed') ?? ''),
          pet_colour: String(fields.get('colour') ?? ''),
          pet_description: String(fields.get('description') ?? ''),
        },
        photo: await photoPayload(photo),
      },
    })
    if (saveError || !data) {
      dispatch({
        type: 'failed',
        error: saveError?.message ?? t('missingCase.saveError'),
      })
      return
    }
    setDraft({ id: data.id, petId: data.petId, petName })
  ...
  function useCurrentLocation() {
    if (!navigator.geolocation) {
      dispatch({
        ...
        error:
          'Your browser cannot provide a location. Move the pin on the map to set the last-seen location.',
      })
      return
    }
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () =>
        dispatch({
          type: 'validation_failed',
          error:
            'We could not access your location. Allow location access in your browser, then try again, or move the pin on the map.',
        }),
  ...
  async function saveLocation(event: FormEvent<HTMLFormElement>) {
    ...
    const { error: updateError } = await supabase
      .from('missing_cases')
      .update({
        exact_location: `POINT(${longitude} ${latitude})`,
        last_seen_at: new Date(location.seenAt).toISOString(),
        last_seen_description: location.label.trim() || null,
        status: 'published',
  ...
  return (
    <div className="form-shell">
      <SimpleHeader onExit={exitFlow} />
      <main className="flow-layout">
        <Progress
          label={t('missingCase.progress')}
          total={2}
          current={current}
        />
        <section className="form-intro">
          <p className="eyebrow">{t('missingCase.eyebrow')}</p>
          <h1>
            {workflow.stage === 'location'
              ? `Where was ${draft?.petName} last seen?`
          ...
          <p>
            {workflow.stage === 'location'
              ? 'Use your current location or move the pin on the map. Only an approximate area is public.'
              : workflow.stage === 'published'
                ? 'You can now share the case with people nearby.'
                : t('missingCase.intro')}
          </p>
        </section>
        {isLoading ? (
          ...
          <section className="auth-card sign-in-required">
            <h2>{t('missingCase.signInTitle')}</h2>
            <p>{t('missingCase.signInBody')}</p>
          ...
          <section className="auth-card saved-pet" role="status">
            ...
            <div>
              <h2>Case published.</h2>
              <p>
                Share the link with people nearby. The public page shows only an
                approximate area.
              </p>
          ...
          >
            <fieldset>
              <legend>
                {t('missingCase.lastSeenQuestion', {
                  petName: draft?.petName ?? '',
                })}
              </legend>
              <p className="location-help">
                Use your current location or move the pin on the map.
              </p>
              <button
              ...
              <p className="pin-note">
                <Icon name="map-pin-2" />
                Move the pin or tap the map to set the last-seen location before
                continuing.
              </p>
              <label>
                Place or landmark
                <input
                  ...
                  placeholder="For example, the south gate of Victoria Park"
                />
              </label>
              <label>
                {t('missingCase.lastSeen')}
                <input
                  type="datetime-local"
                  value={location.seenAt}
                  onChange={(event) =>
                    setLocation({ ...location, seenAt: event.target.value })
                  }
                  required
                />
              </label>
            ...
            <p className="form-privacy">
              ...
              {t('missingCase.exactLocationNote')}
            </p>
            {workflow.error && (
          ...
          <form className="case-form" onSubmit={(event) => void savePet(event)}>
            <fieldset>
              <legend>{t('missingCase.details')}</legend>
              <div className="choice-row" aria-label={t('missingCase.species')}>
                <button
                  type="button"
              ...
              <div className="two-columns">
                <label>
                  ...
                  <input
                    name="breed"
                    maxLength={120}
                    placeholder={t('missingCase.breedHint')}
                  />
                </label>
                <label>
                  {t('missingCase.markings')}
                  <input
                    name="colour"
                    maxLength={120}
                    placeholder={t('missingCase.markingsHint')}
                  />
                </label>
              </div>
              <label>
                {t('missingCase.description')}
                <textarea
                  name="description"
                  maxLength={1500}
                  placeholder={t('missingCase.descriptionHint')}
                  rows={4}
                />
              </label>
            </fieldset>
            <fieldset>
              <legend>{t('missingCase.photo')}</legend>
              <PetPhotoUploadField
