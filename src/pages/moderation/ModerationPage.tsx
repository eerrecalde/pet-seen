...
export function ModerationPage() {
  ...
  const loadReports = useCallback(async () => {
    ...
    const [contentResult, foundResult] = await Promise.all([
      ...
      supabase.from('found_pet_reports').select('id,species,breed,colour,details,custody_status,location_description,found_at,created_at,moderation_status,automated_screening_note,photo:found_pet_photos(source_object_path,display_object_path),link:found_pet_case_links(case_id,case:missing_cases(public_slug,pet:pets(name)))').order('created_at', { ascending: false })
    ])
