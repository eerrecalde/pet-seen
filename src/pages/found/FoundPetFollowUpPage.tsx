...
export function FoundPetFollowUpPage() {
  ...
  const reports = followUp.data?.reports ?? [], messages = followUp.data?.messages ?? {}
  ...
  async function send(event: FormEvent<HTMLFormElement>, reportId: string) {
    ...
    if (!body) return
    try { await sendMessage.mutateAsync({ reportId, body }); event.currentTarget.reset() } catch { /* Query error state is surfaced by the page. */ }
  }
  ...
  return <div className="dashboard-page"><SiteHeader /><main className="dashboard-shell follow-up-shell"><div className="dashboard-intro"><div><p className="eyebrow">{t('found.followUpTitle')}</p><h1>{t('found.followUpPageTitle')}</h1><p>{t('found.followUpPageIntro')}</p></div></div>{followUp.isLoading ? <p>{t('auth.loading')}</p> : followUp.isError ? <p className="form-error">{t('found.followUpLoadError')}</p> : reports.length === 0 ? <section className="dashboard-empty"><h2>{t('found.followUpEmptyTitle')}</h2><p>{t('found.followUpEmptyBody')}</p></section> : <div className="follow-up-list">{reports.map((report) => <article className="follow-up-report" key={report.id}><div><p className="moderation-status">{t(`common.${report.species}`)} · {formatDateTime(report.found_at, i18n.resolvedLanguage as AppLocale)}</p><h2>{[report.colour, report.breed].filter(Boolean).join(' · ') || t('moderation.foundPet')}</h2><p>{t(`found.custody.${report.custody_status}`)}</p></div>{report.link?.status === 'confirmed' ? <Conversation messages={messages[report.id] ?? []} reportId={report.id} sending={sendMessage.isPending} sessionUserId={session.user.id} onSend={send} /> : <p className="follow-up-status">{report.link ? t('found.followUpAwaitingOwner') : t('found.followUpNoMatch')}</p>}</article>)}</div>}</main><SiteFooter /></div>
}
