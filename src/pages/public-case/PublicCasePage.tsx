...
function PublicCaseContent({
  ...
  const descriptionTitle =
    ...
    t('publicCase.descriptionTitle')
  const lastSeen = caseData.last_seen_at
  ...
  return (
    <>
      <CaseMetadata
        title={`${heading} | Pet Seen`}
        description={t('publicCase.leadForPet', { petName: caseData.pet_name })}
        canonicalUrl={publicCaseUrl(caseData.public_slug, locale)}
      ...
      <div className="case-grid">
        ...
        <section className="case-summary">
          ...
          <p className="case-lead">
            {t('publicCase.leadForPet', { petName: caseData.pet_name })}
          </p>
          ...
          <Link className="primary-cta report-cta" to="/sighting/new">
            <Icon name="eye" />
            {t('publicCase.action', { petName: caseData.pet_name })}
          </Link>
          <p className="privacy-note">
            <Icon name="shield-check" />
            {t('publicCase.privacy', { petName: caseData.pet_name })}
          </p>
        </section>
      </div>
      <ShareCasePanel caseData={caseData} locale={locale} heading={heading} />
      <section className="case-details">
        <div>
          <p className="eyebrow">
            {t('publicCase.about', { petName: caseData.pet_name })}
          </p>
        ...
        <div className="map-card">
          <PublicLocationMap
            latitude={caseData.public_latitude}
            longitude={caseData.public_longitude}
            label={t('publicCase.mapLabel', { petName: caseData.pet_name })}
          />
          <p>
            <Icon name="information" />
            {t('publicCase.mapNote', { petName: caseData.pet_name })}
          </p>
        </div>
      </section>
      <ContentReportForm caseSlug={caseData.public_slug} />
    </>
  )
}

function ShareCasePanel({
  caseData,
  locale,
  heading,
}: {
  ...
  const { t } = useTranslation()
  const baseUrl = publicCaseUrl(caseData.public_slug, locale)
  const [status, setStatus] = useState('')
  const shareText = t('publicCase.shareText', { petName: caseData.pet_name })
  async function attributedUrl(
    ...
    return error || !data ? baseUrl : `${baseUrl}?via=${data}`
  }
  async function share() {
    try {
      await navigator.share({
        title: heading,
        text: shareText,
        url: await attributedUrl('web_share'),
      })
  ...
  async function copy() {
    const url = await attributedUrl('copy')
    try {
      await navigator.clipboard.writeText(url)
      setStatus(t('publicCase.copied'))
    } catch {
      ...
      input.remove()
      setStatus(copied ? t('publicCase.copied') : t('publicCase.copyError'))
    }
  }
  ...
  async function poster() {
    const url = await attributedUrl('poster')
    window.open(
      `${url.replace(/\?.*$/, '')}/poster?via=${new URL(url).searchParams.get('via')}`,
      '_blank',
      'noopener,noreferrer',
    )
  }
  return (
    <section className="share-panel" aria-labelledby="share-case-heading">
      <div>
        <p className="eyebrow">{t('publicCase.shareEyebrow')}</p>
        <h2 id="share-case-heading">{t('publicCase.shareTitle')}</h2>
        <p>{t('publicCase.shareIntro')}</p>
      </div>
  ...
  if (state === 'success') {
    return (
      <section className="content-report confirmation" role="status">
        ...
        <div>
          <strong>{t('contentReport.thanksTitle')}</strong>
          <p>{t('contentReport.thanksBody')}</p>
        </div>
      </section>
    )
  }
  ...
  return (
    <section className="content-report">
      <button
        ...
        aria-expanded={open}
      >
        {t('contentReport.trigger')}
      </button>
      {open && (
        <form onSubmit={(event) => void submit(event)}>
          <h2>{t('contentReport.title')}</h2>
          <p>{t('contentReport.intro')}</p>
          <label>
            {t('contentReport.reason')}
            <select
              value={reason}
              onChange={(event) =>
            ...
            >
              <option value="incorrect">
                {t('contentReport.reasons.incorrect')}
              </option>
              <option value="harmful">
                {t('contentReport.reasons.harmful')}
              </option>
              <option value="scam">{t('contentReport.reasons.scam')}</option>
              <option value="other">{t('contentReport.reasons.other')}</option>
            </select>
          </label>
          <label>
            {t('contentReport.details')}
            <textarea
              maxLength={1000}
              value={details}
