...
import {
  ...
  type SightingDraft,
} from '../../features/sighting/workflow'
...
export function SightingPage() {
  ...
  const {
    data: cases = [],
    isError: isCaseOptionsError,
    isPending: isCaseOptionsPending,
  } = usePublicCaseOptionsQuery()
  ...
  const coordinates =
    workflow.location.latitude !== '' && workflow.location.longitude !== ''
      ? {
          latitude: Number(workflow.location.latitude),
          longitude: Number(workflow.location.longitude),
        }
      : null
  const selectedCaseData = cases.find(
    (caseData) => caseData.public_slug === workflow.selectedCase,
  )
  ...
  function saveDraft() {
    window.localStorage.setItem(
      sightingDraftStorageKey,
      JSON.stringify({
        selectedCase: workflow.selectedCase,
        location: workflow.location,
        submissionToken: submissionToken.current,
      } satisfies SightingDraft),
    )
  }
  function chooseCase(caseSlug: string) {
    dispatch({ type: 'choose_case', caseSlug })
  }
  ...
  async function submit(event?: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      dispatch({
        type: 'validation_failed',
        error: t('sighting.invalidLocation'),
      })
      return
    }
    if (!supabase) {
      dispatch({ type: 'validation_failed', error: t('sighting.unavailable') })
      return
    }
    ...
    const { error: submitError } = await supabase.functions.invoke(
      'submit-workflow',
      {
        body: {
          kind: 'sighting',
          payload: {
            selected_case_slug: workflow.selectedCase || null,
            latitude,
            longitude,
            sighted_at: new Date(workflow.location.seenAt).toISOString(),
            place_description: workflow.location.label,
            sighting_details: workflow.location.details,
    ...
    if (submitError) {
      saveDraft()
      if (!navigator.onLine) {
        dispatch({ type: 'submission_saved_offline' })
        return
      }
      dispatch({
        type: 'submission_failed',
        error: submitError.message || t('sighting.submitError'),
      })
      return
    }
      ...
      <div className="form-shell">
        ...
        <main className="flow-layout">
          <section className="auth-card saved-pet" role="status">
            ...
            <div>
              <h1>{t('sighting.thanksTitle')}</h1>
              <p>{t('sighting.thanksBody')}</p>
  ...
  return (
    <div className="form-shell">
      <SimpleHeader />
      <main className="flow-layout">
        <Progress label={t('sighting.progress')} total={2} />
        <section className="form-intro">
          <p className="eyebrow">{t('sighting.eyebrow')}</p>
          <h1>{t('sighting.title')}</h1>
          <p>{t('sighting.intro')}</p>
        </section>
        <form
          className="case-form sighting-form"
          onSubmit={(event) => void submit(event)}
        >
          {workflow.draftRestored && (
            <p className="draft-notice" role="status">
              <Icon name="draft-line" />
              {t('sighting.draftRestored')}
            </p>
          )}
          {!workflow.isOnline && (
            <p className="draft-notice" role="status">
              <Icon name="wifi-off-line" />
              {t('sighting.offline')}
            </p>
          )}
          <fieldset>
            <legend>{t('sighting.petQuestion')}</legend>
            {selectedCaseData ? (
              <SelectedCase
                caseData={selectedCaseData}
                onChange={() => dispatch({ type: 'open_case_picker' })}
                onRemove={() => dispatch({ type: 'clear_case' })}
              />
            ) : (
              <>
                <p className="pet-picker-help">{t('sighting.petHelp')}</p>
                <button
                  className="secondary-button choose-pet-button"
                  type="button"
                  onClick={() => dispatch({ type: 'open_case_picker' })}
                >
                  <Icon name="search-eye" />
                  {t('sighting.choosePet')}
                </button>
              </>
            )}
          </fieldset>
          <fieldset>
            <legend>{t('sighting.whenWhere')}</legend>
            <p className="location-help">{t('sighting.locationHelp')}</p>
            <button
            ...
            <label>
              ...
              <input
                ...
                placeholder={t('sighting.whereHint')}
              />
            </label>
            <label>
              {t('sighting.when')}
              <input
                type="datetime-local"
                value={workflow.location.seenAt}
                onChange={(event) =>
                  dispatch({
                    type: 'update_location',
                    location: {
                      ...workflow.location,
                      seenAt: event.target.value,
                    },
                  })
                }
                required
          ...
          <fieldset>
            <legend>{t('sighting.detailsQuestion')}</legend>
            <label>
              {t('sighting.details')}
              <textarea
                value={workflow.location.details}
                maxLength={1500}
                onChange={(event) =>
                  dispatch({
                    type: 'update_location',
                ...
                placeholder={t('sighting.detailsHint')}
                rows={4}
                required
              />
            </label>
          ...
          {workflow.submission === 'offline' && (
            <div className="submission-error" role="status">
              <p>{t('sighting.offlineSaved')}</p>
              <button
                className="secondary-button"
                type="button"
                onClick={() => void submit()}
          ...
          >
            {workflow.submission === 'saving'
              ? t('sighting.submitting')
              : t('common.submitSighting')}{' '}
      ...
      {workflow.isCasePickerOpen && (
        <CasePicker
          cases={cases}
          isError={isCaseOptionsError}
          isPending={isCaseOptionsPending}
          onChoose={chooseCase}
          onClose={() => dispatch({ type: 'close_case_picker' })}
          onNoMatch={() => dispatch({ type: 'clear_case' })}
        />
      )}
    </div>
  )
...
function SelectedCase({
  ...
  return (
    <div className="selected-case">
      <PetImage
        className="selected-case-photo"
        petName={caseData.pet_name}
        species={caseData.species}
        publicSlug={caseData.public_slug}
      />
      <div>
        <p>{t('sighting.selectedPet')}</p>
        <strong>{caseData.pet_name}</strong>
        <span>
          {[caseData.colour, caseData.breed].filter(Boolean).join(' · ') ||
            t(`common.${caseData.species}`)}
        </span>
      </div>
      <button type="button" className="text-button" onClick={onChange}>
        {t('sighting.changePet')}
      </button>
      <button
        type="button"
        className="remove-selected-case"
        aria-label={t('sighting.removePet', { petName: caseData.pet_name })}
        onClick={onRemove}
      >
...
function CasePicker({
  ...
  return (
    <div
      className="case-picker-backdrop"
      role="presentation"
      onMouseDown={onClose}
    >
      <section
      ...
      >
        <div className="case-picker-heading">
          <div>
            <h2 id="case-picker-title">{t('sighting.choosePetTitle')}</h2>
            <p>{t('sighting.choosePetIntro')}</p>
          </div>
          <button
            className="case-picker-close"
            type="button"
            aria-label={t('sighting.closePicker')}
            onClick={onClose}
          >
            <Icon name="close-line" />
          </button>
        ...
        {isPending ? (
          <p className="pet-picker-help" role="status">
            {t('sighting.casePickerLoading')}
          </p>
        ) : isError ? (
          <p className="location-error" role="alert">
            {t('sighting.casePickerError')}
          </p>
        ) : (
          <div className="case-picker-list">
            {cases.map((caseData) => (
              <button
                type="button"
                className="case-picker-card"
                key={caseData.public_slug}
                onClick={() => onChoose(caseData.public_slug)}
              >
                <PetImage
                  className="case-picker-photo"
                  petName={caseData.pet_name}
                  species={caseData.species}
                  publicSlug={caseData.public_slug}
                />
        ...
        <button className="no-match-button" type="button" onClick={onNoMatch}>
          {t('sighting.noMatch')}
        </button>
      </section>
    </div>
