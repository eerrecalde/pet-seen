...
type OwnerSighting = SightingMapPoint & { case_id: string | null, seen_at: string, location_description: string | null, details: string | null }
...
function OwnerDashboardPage() {
  ...
  const loadCases = useCallback(async () => {
    ...
    if (caseIds.length > 0) {
      const { data: sightingData, error: sightingsError } = await supabase.from('owner_case_sightings').select('id,case_id,seen_at,location_description,details,latitude,longitude').in('case_id', caseIds).order('seen_at', { ascending: false })
...
function OwnerCaseCard({ caseData, sightings, editing, locale, saving, onEdit, onCancel, onSave, onStatus }: { caseData: OwnerCase, sightings: OwnerSighting[], editing: boolean, locale: AppLocale, saving: boolean, onEdit: () => void, onCancel: () => void, onSave: (event: FormEvent<HTMLFormElement>, caseData: OwnerCase) => Promise<void>, onStatus: (caseData: OwnerCase, status: Extract<OwnerCaseStatus, 'published' | 'closed' | 'reunited'>) => Promise<void> }) {
  const { t } = useTranslation()
...
function SightingPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const { error: submitError } = await supabase.rpc('submit_sighting', { selected_case_slug: selectedCase || null, latitude, longitude, sighted_at: new Date(location.seenAt).toISOString(), place_description: location.label, sighting_details: location.details })
