...
function ShareCasePanel({ caseData, locale, heading }: { caseData: PublicCase, locale: AppLocale, heading: string }) {
  const { t } = useTranslation(); const url = publicCaseUrl(caseData.public_slug, locale); const [status, setStatus] = useState(''); const shareText = t('publicCase.shareText', { petName: caseData.pet_name }); const whatsAppUrl = `https://wa.me/?text=${encodeURIComponent(`${shareText} ${url}`)}`
  async function share() { try { await navigator.share({ title: heading, text: shareText, url }); setStatus(t('publicCase.shared')) } catch (error) { if ((error as DOMException).name !== 'AbortError') setStatus(t('publicCase.shareError')) } }
  async function copy() { try { await navigator.clipboard.writeText(url); setStatus(t('publicCase.copied')) } catch { const input = document.createElement('textarea'); input.value = url; input.style.position = 'fixed'; input.style.opacity = '0'; document.body.append(input); input.select(); const copied = document.execCommand('copy'); input.remove(); setStatus(copied ? t('publicCase.copied') : t('publicCase.copyError')) } }
  return <section className="share-panel" aria-labelledby="share-case-heading"><div><p className="eyebrow">{t('publicCase.shareEyebrow')}</p><h2 id="share-case-heading">{t('publicCase.shareTitle')}</h2><p>{t('publicCase.shareIntro')}</p></div><div className="share-actions"><button className="secondary-button" type="button" onClick={() => void copy()}><Icon name="file-copy" />{t('publicCase.copyLink')}</button>{typeof navigator !== 'undefined' && 'share' in navigator && <button className="secondary-button" type="button" onClick={() => void share()}><Icon name="share-line" />{t('publicCase.share')}</button>}<a className="secondary-button" href={whatsAppUrl} target="_blank" rel="noreferrer"><Icon name="whatsapp" />{t('publicCase.whatsApp')}</a><Link className="secondary-button" to={`/find/${caseData.public_slug}/poster`} target="_blank"><Icon name="printer" />{t('publicCase.poster')}</Link></div>{status && <p className="share-status" role="status">{status}</p>}</section>
}
...
function OwnerDashboardPage() {
  ...
  const loadCases = useCallback(async () => {
    if (!supabase || !session) return
    setState('loading')
    const { data, error } = await supabase.from('missing_cases').select('id,public_slug,status,title,last_seen_at,last_seen_description,closed_at,published_at,reunion_reason,reunion_pet_seen_attributed,pet:pets(id,name,species,breed,colour,description)').eq('owner_id', session.user.id).order('created_at', { ascending: false })
    if (error) { setState('error'); return }
    ...
    if (caseIds.length > 0) {
      const { data: sightingData, error: sightingsError } = await supabase.from('owner_case_sightings').select('id,case_id,seen_at,location_description,details,report_status,latitude,longitude').in('case_id', caseIds).order('seen_at', { ascending: false })
