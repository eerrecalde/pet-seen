...
function OwnerCaseCard({ caseData, photoUrl, sightings, editing, locale, saving, onEdit, onCancel, onSave, onStatus, onReviewSighting }: { caseData: OwnerCase, photoUrl?: string, sightings: OwnerSighting[], editing: boolean, locale: AppLocale, saving: boolean, onEdit: () => void, onCancel: () => void, onSave: (event: FormEvent<HTMLFormElement>, caseData: OwnerCase) => Promise<void>, onStatus: (caseData: OwnerCase, status: Extract<OwnerCaseStatus, 'published' | 'closed' | 'reunited'>, reunion?: { reason: NonNullable<OwnerCase['reunion_reason']>, attributed: boolean }) => Promise<void>, onReviewSighting: (sighting: OwnerSighting, status: Exclude<SightingReportStatus, 'pending'>) => Promise<void> }) {
  const { t: translated, i18n } = useTranslation()
  const t = dashboardTranslation(translated, i18n.resolvedLanguage)
