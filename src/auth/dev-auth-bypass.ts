...
export function readDevAuthBypass(): DevAuthBypass | null {
  ...
  const identityType = /^\S+@\S+\.\S+$/.test(identity) ? 'email' : /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(identity) ? 'userId' : null
  if (!identityType || !['owner', 'moderator', 'administrator'].includes(role)) return null
...
export async function signInWithDevBypass(bypass: DevAuthBypass) {
  ...
  const { data, error } = await supabase.functions.invoke('dev-auth-bypass', { body: { [bypass.identityType === 'email' ? 'email' : 'userId']: bypass.identity, role: bypass.role } })
  if (error) throw error
  if (!data || typeof data.email !== 'string' || typeof data.password !== 'string') throw new Error('The local development sign-in could not be completed.')
