...
Place search uses `geocode-location`, an Edge Function that holds the dedicated `GEOAPIFY_GEOCODING_API_KEY`; never add that key to a `VITE_` variable. Keep the browser MapTiler style key origin-restricted to the production hostname and approved subdomains, and configure the Geoapify key with the narrowest available server-side/IP restriction. The function normalises queries, caches public provider results for 24 hours, and limits uncached provider calls to 20 per client and 300 globally per five minutes. Search runs only when a visitor selects Search or presses Enter, and a rate-limit or provider failure must leave manual pin placement available.

Before beta, configure a Geoapify quota/spend alert at 80% and alert routing to the on-call channel. Review geocoding usage weekly and rotate the server key immediately after suspected misuse. During a sustained Geoapify outage or quota exhaustion, keep manual-pin reporting enabled and disable search by removing the geocoding secret. MapTiler remains the browser-map provider; do not proxy or cache map tiles.
