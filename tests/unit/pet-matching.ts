...
const genericBreedValues = new Set([
  'unknown',
  'mixed',
  'other',
  'crossbreed',
...
const ignoredColourTokens = new Set([
  'a',
  'an',
  'and',
  'or',
  ...
  return [
    ...new Set(
      value
        ...
        .filter((token) => token && !ignoredColourTokens.has(token)),
    ),
  ].sort()
}

export function hasSafePartialBreedMatch(
  ...
  return (
    shorter.length >= 8 &&
    !genericBreedValues.has(shorter) &&
    longer.includes(shorter)
  )
}

export function scoreAttributeEvidence({
  reportBreed,
  caseBreed,
  reportColour,
  caseColour,
}: {
  reportBreed?: string | null
  caseBreed?: string | null
  reportColour?: string | null
  caseColour?: string | null
}) {
  const normalizedReportBreed = canonicalBreed(reportBreed)
  const normalizedCaseBreed = canonicalBreed(caseBreed)
  const breedPoints =
    normalizedReportBreed && normalizedReportBreed === normalizedCaseBreed
      ? 10
      : hasSafePartialBreedMatch(reportBreed, caseBreed)
        ? 5
        : 0
  const reportTokens = colourTokens(reportColour)
  const caseTokens = colourTokens(caseColour)
  const exactColour =
    reportTokens.length > 0 && reportTokens.join('|') === caseTokens.join('|')
  const partialColour =
    !exactColour && reportTokens.some((token) => caseTokens.includes(token))
