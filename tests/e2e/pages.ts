...
export class MissingCasePage {
  ...
  async open() { await this.page.goto('/lost/new'); await expect(this.page.getByRole('heading', { name: 'Tell us about your pet.' })).toBeVisible() }
...
export class PublicCasePage {
  ...
  async open(slug: string, petName: string) {
    ...
    await expect(this.page.getByRole('heading', { name: `${petName} is missing` })).toBeVisible()
  }
}
...
export class DashboardPage {
  ...
  async open() { await this.page.goto('/dashboard'); await expect(this.page.getByRole('heading', { name: 'Manage your missing-pet cases.' })).toBeVisible() }
  ...
  async confirmSightingAndReunite() {
    ...
    await expect(this.page.getByRole('heading', { name: 'Tell us about the reunion.' })).toBeVisible()
    await this.page.getByLabel('How was your pet reunited?').selectOption('seen_after_report')
