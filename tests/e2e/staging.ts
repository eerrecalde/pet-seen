...
export async function createStagingOwner(): Promise<StagingOwner> {
  ...
  if (createError || !created.user)
    throw createError ?? new Error('Could not create the Playwright owner.')
  ...
  if (error || !data.session) {
    ...
    throw (
      error ??
      new Error('Could not create a browser session for the Playwright owner.')
    )
  }
...
export async function createStagingStaff(): Promise<StagingStaff> {
  ...
  if (createError || !created.user)
    throw (
      createError ?? new Error('Could not create the Playwright staff member.')
    )
  const { error: roleError } = await admin
    .from('user_roles')
    .insert({ user_id: created.user.id, role: 'moderator' })
  if (roleError) {
    await admin.auth.admin.deleteUser(created.user.id)
    throw roleError
  }
  ...
  if (error || !data.session) {
    ...
    throw (
      error ??
      new Error(
        'Could not create a browser session for the Playwright staff member.',
      )
    )
  }
  return { id: created.user.id, session: data.session }
...
export async function createStagingFoundPetReport(
  options: {
    foundAt?: string
    lifecycleStatus?: 'active' | 'resolved' | 'expired'
    lifecycleChangedAt?: string
    withPhoto?: boolean
    submittedBy?: Session
  } = {},
): Promise<StagingFoundPetReport> {
  const details = `Playwright found-pet lifecycle ${crypto.randomUUID()}`
  const submissionToken = crypto.randomUUID()
  const submitter = options.submittedBy
    ? createClient(url, anonKey, {
        auth: { persistSession: false, autoRefreshToken: false },
        global: {
          headers: {
            authorization: `Bearer ${options.submittedBy.access_token}`,
          },
        },
      })
    : publicClient
  const { data: id, error } = await submitter.rpc('submit_found_pet_report', {
    found_species: 'dog',
    found_breed: 'Test breed',
    found_colour: 'Black and white',
    found_details: details,
    found_custody_status: 'with_reporter',
    latitude: 51.5074,
    longitude: -0.1278,
    found_time: options.foundAt ?? new Date().toISOString(),
    place_description: 'Playwright test location',
  ...
  if (error || !id)
    throw (
      error ?? new Error('Could not create the Playwright found-pet report.')
    )
  ...
  const { error: updateError } = await admin
    ...
    .update({
      ...
      lifecycle_reason: lifecycleStatus === 'active' ? null : 'test',
      lifecycle_changed_at:
        options.lifecycleChangedAt ?? new Date().toISOString(),
    })
    .eq('id', id)
  if (updateError) {
    await admin.from('found_pet_reports').delete().eq('id', id)
    throw updateError
  }
  if (!options.withPhoto)
    return { id, details, sourceObjectPath: null, displayObjectPath: null }
  ...
  const { error: uploadError } = await bucket.upload(sourceObjectPath, jpeg, {
  ...
  const { error: attachError } = await submitter.rpc('attach_found_pet_photo', {
    target_report_id: id,
    submission_token: submissionToken,
  })
  if (attachError) {
    await bucket.remove([sourceObjectPath])
    await admin.from('found_pet_reports').delete().eq('id', id)
    throw attachError
  }
...
export async function foundPetAuditEvents(
  reportId: string,
...
export async function deleteStagingFoundPetReport(
  report: StagingFoundPetReport | undefined,
) {
  if (!report || !(await foundPetReportExists(report.id))) return
  if (report.sourceObjectPath || report.displayObjectPath)
    await admin.storage
      .from('found-pet-photos')
      .remove(
        [report.sourceObjectPath, report.displayObjectPath].filter(
